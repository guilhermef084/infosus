...
$_SG['paginaLogin'] = 'index2.php'; // Página de login
$_SG['tabela'] = 'tbhospital'; // Nome da tabela do db onde os usuários são cadastrados.
...
function validaUsuario($usuario, $senha){
    ...
    $ncodHospitalLogado = addslashes($codHospitalLogado);
    ...
    $query = mysqli_query($_SG['conexao'], "SELECT `loginHospital`, `senhaHospital`, `codHospital` FROM `" .
        $_SG['tabela'] . "` WHERE " . $cS . " `loginHospital` = '" . $nusuario . "' AND " . $cS .
        " `senhaHospital` = '" . $nsenha . "' LIMIT 1");
    ...
    if (empty($resultado)) {
        ...
        $_SESSION['usuarioID'] = $resultado['codHospital']; // Pega o valor da coluna 'id do registro encontrado no MySQL
        $_SESSION['usuarioLogin'] = $resultado['loginHospital']; // Pega o valor da coluna 'nome' do registro encontrado no MySQL
        $_SESSION['usuarioSobrenome'] = $resultado['senhaHospital']; // Pega o valor da coluna 'nome' do registro encontrado no MySQL
        $_SESSION['log'] = 1;
        // Verifica a opção se sempre validar o login
        if ($_SG['validaSempre'] == true) {
            // Definimos dois valores na sessão com os dados do login
            $_SESSION['usuarioLogin'] = $usuario;
            $_SESSION['usuarioSobrenome'] = $senha;
            $_SESSION['usuarioID'] = $codHospitalLogado;
            $_SESSION['log'] = 1;
        }
        return true;
    }
...
function protegePagina(){
    global $_SG;
    if (!isset($_SESSION['usuarioID']) or !isset($_SESSION['usuarioLogin']) or !isset($_SESSION['usuarioSobrenome'])) {
        ...
        if ($_SG['validaSempre'] == true) {
            ...
            if (!validaUsuario($_SESSION['usuarioLogin'], $_SESSION['usuarioSenha'], $_SESSION['usuarioID'])) {
...
/**
 * Função para negar url forçada
 */
function acessoNegado($location){
    global $_SG;
    // Remove as variáveis da sessão (caso elas existam)
    unset($_SESSION['usuarioID'], $_SESSION['usuarioNome'], $_SESSION['usuarioLogin'],
        $_SESSION['usuarioSenha']);
    // Manda pra tela de logino
    if($location == 1){
