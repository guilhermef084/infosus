<?php 
    ...
    $queryData = $con->query("SELECT * from tbMedico where codMedico=$idMedico and codHospital=$codHospitalLogado");
    
    while($reg = $queryData->fetch_array()){
        $codMedico = $reg['codMedico'];
        $nomeMedico = $reg['nomeMedico'];
        $sobreNomeMedico = $reg['sobreNomeMedico'];
        $crmMedico = $reg['crmMedico'];
        $cpfMedico = $reg['cpfMedico'];
        $filePic = $reg['fotoMedico'];
    }

    //consulta das especialidade do medico
    $queryEspecialidade = $con->query("SELECT nomeEspecialidade,tbespecialidade.codEspecialidade,codMedicoEspecialidade,tbespecialidade.codHospital
                            FROM tbEspecialidade
                            INNER JOIN tbMedicoEspecialidade
                            ON tbEspecialidade.codEspecialidade = tbMedicoEspecialidade.codEspecialidade
                            where codMedico=$codMedico");
...
<body>
    ...
    <div id="main-wrapper" data-navbarbg="skin6" data-theme="light" data-layout="vertical" data-sidebartype="full" data-boxed-layout="full">
       ...
       <aside class="left-sidebar" data-sidebarbg="skin5">
                        ...
                        <div id="listar">
                            <form id="frm_alterar">
                                <div class="row bg-white pb-5 pt-3 mb-3 border border-primary">
                                    ...
                                    <div class="col-sm-12">
                                        <label for="idMedico" class="col-form-label">Código do Médico:</label>
                                        <input type="text" class="form-control" value="<?php echo $codMedico; ?>" disabled/>
                                        <input id="idMedico" type="hidden" class="form-control" name="idMedico" value="<?php echo $codMedico; ?>"/>
                                    </div>

                                    <div class="col-sm-6">
                                        <label for="nomeMedico" class="col-form-label">Nome:</label>
                                        <input id="nomeMedico" type="text" class="form-control" name="txtNomeMedico" value="<?php echo $nomeMedico; ?>"/>
                                    </div>

                                    <div class="col-sm-6">
                                        <label for="sobreNome" class="col-form-label">Sobrenome:</label>
                                        <input id="sobreNome" type="text" class="form-control" name="txtSobreNomeMedico" value="<?php echo $sobreNomeMedico; ?>"/>
                                    </div>

                                    <div class="col-sm-6">
                                        <label for="cpfMedico" class="col-form-label">CPF:</label>
                                        <input id="cpfMedico" type="text" class="form-control" name="txtCpfMedico" value="<?php echo $cpfMedico; ?>" disabled/>
                                    </div>

                                    <div class="col-sm-6">
                                        <label for="crmMedico" class="col-form-label">CRM:</label>
                                        <input id="crmMedico" type="text" class="form-control" name="txtCpfMedico" value="<?php echo $crmMedico; ?>" disabled/>
                                    </div>

                                    <div class="col-sm-6">
                                        <label lass="col-form-label" style="color:white;">1</label>
                                        <button type="button" class="form-control btn btn-primary" data-target=".modalEspecialidade" data-toggle="modal">Ver especialidades</button>
                                    </div>
                                </div>
        ...
        <!-- modal especialidade-->
        <div class="modal fade modalEspecialidade" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="false" data-target=".modal-footer">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalLabel">Especialidades cadastradas neste médico:</h5>
                        ...
                        <div class="modal-body">
                            <div class="container">
                                <div id="tabela_esp" class="col-sm-12 card">
                                    <label class="col-form-label">Especialidades:</label>
                                    <?php require_once("detalhesMedicoEsp.php"); ?>
                                </div>
                            </div>
                        </div>
    ...
    <script>
        $('#tabela_dados tbody td a.excluir').click(function(e){
            ...
            $.ajax({
                type:"POST",
                data:"idMedicoEspecialidade="+ id,
                url:"../crud/deleteMedicoEspecialidade.php",
                async:false
            }).done(function(data){
